<template>
    <el-card>
        ...
        <el-table :data="data?.assetTransactions" stripe style="width: 100%; margin: auto;"
...
const { data, error, status } = await useAsyncData(async () => {
    ...
    if (!issuerId.value) {
        throw new Error('[AssetTransaction] issuerId prop not defined');
    }

    const assetTransactionsRaw = await connection.value.api.Transaction.getTransactions({ senderId: issuerId.value, type: 101 });
    if (!assetTransactionsRaw.success) {
        throw new Error('')
    }

    const assetTransactions = assetTransactionsRaw.transactions;
    const count = assetTransactionsRaw.count;
