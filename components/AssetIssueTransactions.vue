<template>
    ...
    <el-card>
        ...
        <el-table :data="data?.issueTransactions" stripe style="width: 100%; margin: auto;" v-loading="status !== 'success'">
...
const { data, error, status } = await useAsyncData(async () => {
    ...
    if (!issuerId.value) {
        throw new Error('[AsssetIssueTransactions] props issuerId is not defined');
    }

    const issueTransactionsRaw = await connection.value.api.Transaction.getTransactions({ senderId: issuerId.value, type: 102 });
    if (!issueTransactionsRaw.success) {
        throw new Error('failed to fetch ');
    }

    return {
        issueTransactions: issueTransactionsRaw.transactions,
        count: issueTransactionsRaw.count,
    };
}, {
    lazy: true,
    watch: [issuerId],
